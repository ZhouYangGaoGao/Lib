 ...
 */
public class CCRBindingRecyclerViewAdapter<M, B extends ViewDataBinding> extends RecyclerView.Adapter<CCRBindingViewHolder<B>> {
     ...
     */
    public CCRBindingRecyclerViewAdapter() {
    }

    /**
     * 默认的布局文件资源 id
     */
    public CCRBindingRecyclerViewAdapter(@LayoutRes int defaultItemLayoutId) {
    ...
    @Override
    public CCRBindingViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new CCRBindingViewHolder(this, DataBindingUtil.inflate(getLayoutInflater(parent), viewType, parent, false));
    }

    @Override
    public void onBindViewHolder(CCRBindingViewHolder<B> viewHolder, int position) {
        // 在设置值的过程中忽略选中状态变化
        ...
        binding.setVariable(com.luck.picture.lib.BR.viewHolder, viewHolder);
        binding.setVariable(com.luck.picture.lib.BR.uiHandler, mUiHandler);
        binding.setVariable(com.luck.picture.lib.BR.statusModel, mStatusModel);
        binding.setVariable(com.luck.picture.lib.BR.model, model);
    ...
    public void addNewData(List<M> data) {
        if (CCRBaseAdapterUtil.isListNotEmpty(data)) {
            mData.addAll(0, data);
    ...
    public CCRHeaderAndFooterAdapter getHeaderAndFooterAdapter() {
        if (mHeaderAndFooterAdapter == null) {
            synchronized (CCRBindingRecyclerViewAdapter.this) {
                if (mHeaderAndFooterAdapter == null) {

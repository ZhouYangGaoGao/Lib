 ...
 * 并且使用{@link GroupAdapter},就可以实现列表头部吸顶功能。
...
public class StickyHeaderLayout extends FrameLayout {
    ...
    private void updateStickyView(boolean imperative) {
        ...
        if (adapter instanceof GroupAdapter) {
            GroupAdapter gAdapter = (GroupAdapter) adapter;
     ...
     * 注册adapter刷新监听
     */
    private void registerAdapterDataObserver(GroupAdapter adapter) {
     ...
     * @return 返回偏移量。
     */
    private float calculateOffset(GroupAdapter gAdapter, int firstVisibleItem, int groupPosition) {
        int groupHeaderPosition = gAdapter.getPositionForGroupHeader(groupPosition);
        if (groupHeaderPosition != -1) {

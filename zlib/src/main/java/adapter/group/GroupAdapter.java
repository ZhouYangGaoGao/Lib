 ...
 * 因为当分组列表发生变化时，需要及时更新分组列表的组结构{@link GroupAdapter#mStructures}
 */
public abstract class GroupAdapter<T>
    ...
    private int itemLayout = R.layout.item_text
            , heardLayout = R.layout.item_text
            , footLayout = R.layout.item_text;

    public GroupAdapter(List<T> mData, int... itemLayout) {
        this(BApp.app().act(), false);
        this.group = mData;
        if (itemLayout.length > 0)
            this.itemLayout = itemLayout[0];
        if (itemLayout.length > 1)
    ...
    public GroupAdapter(Context context, boolean useBinding) {
        mContext = context;
...
//            }
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
        ...
        if (type == TYPE_HEADER) {
            onBindHeaderViewHolder(((BaseViewHolder) holder).setGroupPosition(groupPosition), group.get(groupPosition));
        } else if (type == TYPE_FOOTER) {
            onBindFooterViewHolder(((BaseViewHolder) holder).setGroupPosition(groupPosition), group.get(groupPosition));
        } else if (type == TYPE_CHILD) {
            int childPosition = getChildPositionForPosition(groupPosition, position);
            onBindChildViewHolder(((BaseViewHolder) holder).setGroupPosition(groupPosition), group.get(groupPosition), childPosition);
        }
    }
    ...
    private void structureChanged() {
        ...
        for (int i = 0; i < groupCount; i++) {
            mStructures.add(new GroupStructure(hasHeader(i), hasFooter(i), getChildrenCount(group.get(i))));
        }
        isDataChanged = false;
    }
    ...
    public void notifyGroupInserted(int groupPosition) {
        GroupStructure structure = new GroupStructure(hasHeader(groupPosition),
                hasFooter(groupPosition), getChildrenCount(group.get(groupPosition)));
        if (groupPosition < mStructures.size()) {
    ...
    public void notifyGroupRangeInserted(int groupPosition, int count) {
        ...
        for (int i = 0; i < count; i++) {
            GroupStructure structure = new GroupStructure(hasHeader(i),
                    hasFooter(i), getChildrenCount(group.get(i)));
            list.add(structure);
        }
    ...
    public void notifyChildrenInserted(int groupPosition) {
        if (groupPosition < mStructures.size()) {
            ...
            int itemCount = getChildrenCount(group.get(groupPosition));
            if (itemCount > 0) {
    ...
    public void onBindHeaderViewHolder(BaseViewHolder holder, T t) {
    }

    public void onBindFooterViewHolder(BaseViewHolder holder, T t) {
    }

    public abstract void onBindChildViewHolder(BaseViewHolder holder, T t, int childPosition);

...
public class BPermissionFragment extends Fragment implements EasyPermissions.PermissionCallbacks {
    ...
    public static void request(OnPermissionListener listener, String... mPerms) {
        if (fragment == null) {
            ...
            BApp.app().act().getWindow()
                    .getDecorView().setId(R.id.mRootView);
            ((AppCompatActivity) BApp.app().act()).getSupportFragmentManager()
                    .beginTransaction().replace(BApp.app().act().getWindow()
                    .getDecorView().getId(), fragment).commit();
        } else {
            fragment.listener = listener;
            if (mPerms.length > 0)
                fragment.mPerms = mPerms;
    ...
    private BPermissionFragment() {
    }

    private BPermissionFragment(int contentLayoutId) {

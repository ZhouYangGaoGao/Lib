...
public class NetUtil {
    ...
    public static boolean isNetworkAvailable() {
        ConnectivityManager manager = (ConnectivityManager) BApp.app().currentActivity().getSystemService(
    ...
    public static boolean isServiceWork(String serviceName) {
        ...
        ActivityManager manager = (ActivityManager) BApp.app().getSystemService(Context.ACTIVITY_SERVICE);
        //关于正在运行的服务数量拿不到，以前设置40时在Honor8（华为手机）7.0系统，就没有拿到自己的服务，尴尬了，目前没找到好的处///理方式，就把服务设置成100，这个手机跑这的服务74个，有人说100个手机会卡的不行，不知道，但这个手机很流畅
        List<ActivityManager.RunningServiceInfo> runningServices = manager.getRunningServices(100);
        if (runningServices.size() <= 0)
            return false;
        for (int i = 0; i < runningServices.size(); i++) {
            if (serviceName.equals(runningServices.get(i).service.getClassName().toString())) {
                iswork = true;
                break;
            }
        }

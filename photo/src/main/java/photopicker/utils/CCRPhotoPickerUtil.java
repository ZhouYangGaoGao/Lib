...
public class CCRPhotoPickerUtil {
    ...
    public static int getScreenWidth() {
        WindowManager windowManager = (WindowManager) CCRBaseAdapterUtil.getApp().getSystemService(Context.WINDOW_SERVICE);
    ...
    public static void show(CharSequence text) {
        if (!TextUtils.isEmpty(text)) {
            if (text.length() < 10) {
                Toast.makeText(CCRBaseAdapterUtil.getApp(), text, Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(CCRBaseAdapterUtil.getApp(), text, Toast.LENGTH_LONG).show();
            }
        }
    }
    ...
    public static void show(@StringRes int resId) {
        show(CCRBaseAdapterUtil.getApp().getString(resId));
    }
    ...
    public static void showSafe(@StringRes int resId) {
        showSafe(CCRBaseAdapterUtil.getApp().getString(resId));
    }
}